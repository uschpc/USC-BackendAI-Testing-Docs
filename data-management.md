...
For data storage that is persistent across multiple sessions, create a *storage folder* within the Topanga platform. Cost is based on active storage so use when you need to temporarily store data between sessions. For longer term data storage, it may be more cost efficient to use the /project2 or /scratch1 [filesystems](#hpc-cluster-filesystem).
...
An existing storage folder can be mounted during the session creation process. 
...
The /home directory for every session is located at `/home/work`. Data here is temporarily stored on the Topanga file system and will be deleted when the session ends. **Do not store important or long-term data here.**
